...
public class TemplateManager {
	...
	public static InputStream getTemplateInputStream(String templateDir,
			String path) throws IOException {
		URL url = BUNDLE.getEntry(templateDir + "/" + path);
		return url.openStream();
	}

	public static void createFromTemplate(IContainer destinationFolder,
			String templateDir) throws CoreException, IOException {
		createFromTemplate(destinationFolder, templateDir, templateDir);
	}

	private static void createFromTemplate(IContainer destinationFolder,
			String templateDir, String path) throws CoreException, IOException {
		...
		for (; enumeration
			...
			if (entryPath.endsWith("/")) {
				String folderPath = entryPath
						.substring(templateDir.length() + 1);
				IFolder folder = destinationFolder.getFolder(new Path(
						folderPath));
				if (!folder.exists()) {
